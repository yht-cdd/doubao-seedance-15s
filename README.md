...
- ✅ **无水印生成** — 通过 `declarativeNetRequest` 自动去除视频/图片水印
...
│   └── icon128.png
├── src/
│   ├── inject.js          # MAIN 世界脚本（hook fetch/XHR 修改 duration）
│   ├── content.js         # 内容脚本（注入 15s 菜单项 + hook 恢复）
│   ├── background.js      # 后台服务（去水印规则管理）
│   ├── popup.html         # 弹窗界面
│   ├── popup.js           # 弹窗逻辑
│   ├── styles.css         # 注入样式
│   └── rules.json         # 静态去水印规则
...
- 本插件仅用于学习和研究目的
- 15s 时长功能依赖豆包后端是否支持
- 无水印功能的效果取决于豆包的 API 实现
- 请勿用于商业用途或违反豆包的使用条款
- 篡改请求参数可能导致账号额度清零或功能封禁
